...
def get_objective_function(trainx,trainy,loss_type, regularizer_type, loss_weight):
	...
	def objective_function(weights):
		...
		loss +=  loss_weight*loss_function(targets, outputs)/len(trainx)
		if regularizer_type != None:
...
def train(data_loader, loss_type, regularizer_type, loss_weight):
	...
	for i in range(num_epochs):
		...
		print regularizer_type
		if(regularizer_type != None): 
			print "regulariser detected"
	#plt.plot(loss_arr)
	print regularizer_type
	if(regularizer_type != None): 
		print "regulariser detected"  
		filename = loss_type+regularizer_type+str(loss_weight)+".npy"
	else:
		filename = loss_type+str(loss_weight)+".npy" 
